    ...
    public partial class editarProducto : ContentPage
    {
        ...
        private async void actualizar(object sender, EventArgs e)
        {
            ...
            var registro = new Modelos.Productos
            {
                ...
                PreciodeVenta = int.Parse(precioDeVenta.Text),
                Cantidad = int.Parse(cantidad.Text),
                PreciodeCosto = int.Parse(precioDeCosto.Text),
                Descripcion = descripcion.Text,
                FotoProducto = foto.Text
            };

            db.Table<Modelos.Clientes>();
            db.Update(registro);
            await DisplayAlert("Mensaje", "Producto actualizado", "Aceptar");
        ...
        async private void eliminar(object sender, EventArgs e)
        {
            ...
            int MiId = int.Parse(IdProducto.Text);
            ...
            if (respuesta == true)
            {
                ...
                await DisplayAlert("Mensaje", "Producto eliminado", "Aceptar");
                await Navigation.PopAsync();
            }
        }
    }
